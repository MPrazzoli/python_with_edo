...
# Project's imports
from _2_api_moduls._2_0_stock_dataframe_class import StockClass, StockListClass, ImportantDatesClass, IndexClass, IndicesListClass
...
######################### EDP
investpy_idx_python = pd.read_csv(data_path + '/investpy_indices.csv', usecols=['symbol', 'class', 'name'])
investpy_idx_python.set_index('symbol', inplace=True)
#########################
...
######################### EDP
investpy_idx_list_object = IndicesListClass(idx_list=list(investpy_idx_python.index),
                                            idx_class=list(investpy_idx_python.values[:, 0]),
                                            idx_name=list(investpy_idx_python.values[:, 1]))
#########################
...
# of our _6_analysis period ( UnitedStates calendar is the Quanlib object to manage trading days and
...
# (take a look at _2_0_stock_dataframe_class.py for more information)
stock_object_dictionary = {'{0}'.format(ticker): StockClass(ticker=ticker, isin=ticker_list_object.isin_list[i],
                                                            exchangeid=ticker_list_object.exchangeId_list[i]) for
...
######################### EDP
tmp = investpy_idx_list_object.idx_name[4]



idx_object_dictionary = {'{0}'.format(symbol): IndexClass(symbol=symbol, name=investpy_idx_list_object.idx_name[i],
                                                          type=investpy_idx_list_object.idx_class[i]) for
                         i, symbol in enumerate(investpy_idx_list_object.idx_list)}


for symbol in investpy_idx_list_object.idx_list:

    try:
        idx_object_dictionary['{0}'.format(symbol)].history = investpy.get_index_historical_data(
                        index=symbol, country=market, from_date='01/01/2021', #important_dates.start_date_investingcom,
                        to_date='26/03/2021') #important_dates.end_date_investingcom)
    except:
        pass
